...
df = df.dropna(subset=['MFE_category', 'MFE_sumif'])  # Drop rows with NaNs in specified columns

GC_fragment = df['MFE_category'].values
fragment_count = df['MFE_sumif'].values
...
df['predicted_MFE_sumif'] = df['MFE_category'].apply(smoothed_function)
df['predicted_MFE_sumif'] = df['predicted_MFE_sumif'].clip(lower=0)  # Ensure non-negative predictions

# Save to CSV (as described in previous instructions)
output_filename = 'smooth_data_with_predictions_MFE.csv'
